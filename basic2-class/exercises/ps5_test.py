...
class ProblemSet5NewsStory(unittest.TestCase):
    ...
    def testNewsStoryGetGuid(self):
        story = NewsStory("test guid", "test title", "test description", "test link", datetime.now())
...
class ProblemSet5(unittest.TestCase):
    ...
    def test1TitleTrigger(self):
        ...
        for trig in [s1, s2]:
            ...
            self.assertTrue(
                trig.evaluate(spaces), "TitleTrigger failed to fire when the words were separated by multiple spaces."
            )
            self.assertTrue(
                trig.evaluate(caps),
            ...
            self.assertFalse(
                trig.evaluate(badorder), "TitleTrigger fired when the words in the phrase appeared out of order."
            )
            self.assertFalse(
                trig.evaluate(nospaces), "TitleTrigger fired when words were not separated by spaces or punctuation."
            )
            self.assertFalse(
                trig.evaluate(nothing), "TitleTrigger fired when none of the words in the phrase appeared."
            )
    ...
    def test2DescriptionTrigger(self):
        ...
        for trig in [s1, s2]:
            self.assertTrue(
                trig.evaluate(cuddly), "DescriptionTrigger failed to fire when the phrase appeared in the description."
            )
            self.assertTrue(
                trig.evaluate(exclaim),
            ...
            self.assertFalse(
                trig.evaluate(badorder), "DescriptionTrigger fired when the words in the phrase appeared out of order."
            )
            self.assertFalse(
            ...
            self.assertFalse(
                trig.evaluate(nothing), "DescriptionTrigger fired when none of the words in the phrase appeared."
            )
    ...
    def test3altBeforeAndAfterTrigger(self):
        ...
        self.assertTrue(
            s1.evaluate(just_now), "BeforeTrigger failed to fire on news happened right before specified time"
        )

        self.assertFalse(
            s1.evaluate(in_a_bit), "BeforeTrigger fired to fire on news happened right after specified time"
        )
        ...
        self.assertFalse(
            s2.evaluate(just_now), "BeforeTrigger fired to fire on news happened right before specified time"
        )
